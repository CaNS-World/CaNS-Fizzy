    ...
    do k=1,n(3)
      do j=1,n(2)
        do i=1,n(1)
          ...
          wr_u = sigma(1)*(1.d0+tauP_r/(br_u+eps)+0.*((1.d0/dxi)**lmbd)*((br_u+eps)/(tauP_r+eps)))
          wr_c = sigma(2)*(1.d0+tauP_r/(br_c+eps)+0.*((1.d0/dxi)**lmbd)*((br_c+eps)/(tauP_r+eps)))
          wl_u = sigma(1)*(1.d0+tauP_l/(bl_u+eps)+0.*((1.d0/dxi)**lmbd)*((bl_u+eps)/(tauP_l+eps)))
          wl_c = sigma(2)*(1.d0+tauP_l/(bl_c+eps)+0.*((1.d0/dxi)**lmbd)*((bl_c+eps)/(tauP_l+eps)))
          wr_u = wr_u/(wr_u+wr_c)
          wr_c = wr_c/(wr_u+wr_c)
          wl_u = wl_u/(wl_u+wl_c)
          wl_c = wl_c/(wl_u+wl_c)
          ...
          tauP_r = abs(0.5d0*(br_u+br_c)-0.25d0*(ap*(fm-fp)**2+am*(fc-fq)**2))
          tauP_l = abs(0.5d0*(bl_u+bl_c)-0.25d0*(ap*(fl-fc)**2+am*(fm-fp)**2))
          wr_u = sigma(1)*(1.d0+tauP_r/(br_u+eps)+0.*((1.d0/dyi)**lmbd)*((br_u+eps)/(tauP_r+eps)))
          wr_c = sigma(2)*(1.d0+tauP_r/(br_c+eps)+0.*((1.d0/dyi)**lmbd)*((br_c+eps)/(tauP_r+eps)))
          wl_u = sigma(1)*(1.d0+tauP_l/(bl_u+eps)+0.*((1.d0/dyi)**lmbd)*((bl_u+eps)/(tauP_l+eps)))
          wl_c = sigma(2)*(1.d0+tauP_l/(bl_c+eps)+0.*((1.d0/dyi)**lmbd)*((bl_c+eps)/(tauP_l+eps)))
          wr_u = wr_u/(wr_u+wr_c)
          wr_c = wr_c/(wr_u+wr_c)
          wl_u = wl_u/(wl_u+wl_c)
          wl_c = wl_c/(wl_u+wl_c)
          ...
          tauP_r = abs(0.5d0*(br_u+br_c)-0.25d0*(ap*(fm-fp)**2+am*(fc-fq)**2))
          tauP_l = abs(0.5d0*(bl_u+bl_c)-0.25d0*(ap*(fl-fc)**2+am*(fm-fp)**2))
          wr_u = sigma(1)*(1.d0+tauP_r/(br_u+eps)+0.*((1.d0/dzci(k  ))**lmbd)*((br_u+eps)/(tauP_r+eps)))
          wr_c = sigma(2)*(1.d0+tauP_r/(br_c+eps)+0.*((1.d0/dzci(k  ))**lmbd)*((br_c+eps)/(tauP_r+eps)))
          wl_u = sigma(1)*(1.d0+tauP_l/(bl_u+eps)+0.*((1.d0/dzci(k-1))**lmbd)*((bl_u+eps)/(tauP_l+eps)))
          wl_c = sigma(2)*(1.d0+tauP_l/(bl_c+eps)+0.*((1.d0/dzci(k-1))**lmbd)*((bl_c+eps)/(tauP_l+eps)))
          wr_u = wr_u/(wr_u+wr_c)
          wr_c = wr_c/(wr_u+wr_c)
          wl_u = wl_u/(wl_u+wl_c)
          wl_c = wl_c/(wl_u+wl_c)
          ...
          tauP_r = abs(0.5d0*(br_u+br_c)-0.25d0*(ap*(fm-fp)**2+am*(fc-fq)**2))
          tauP_l = abs(0.5d0*(bl_u+bl_c)-0.25d0*(ap*(fl-fc)**2+am*(fm-fp)**2))
          wr_u = sigma(1)*(1.d0+tauP_r/(br_u+eps)+0.*((1.d0/dzfi(k  ))**lmbd)*((br_u+eps)/(tauP_r+eps)))
          wr_c = sigma(2)*(1.d0+tauP_r/(br_c+eps)+0.*((1.d0/dzfi(k  ))**lmbd)*((br_c+eps)/(tauP_r+eps)))
          wl_u = sigma(1)*(1.d0+tauP_l/(bl_u+eps)+0.*((1.d0/dzfi(k-1))**lmbd)*((bl_u+eps)/(tauP_l+eps)))
          wl_c = sigma(2)*(1.d0+tauP_l/(bl_c+eps)+0.*((1.d0/dzfi(k-1))**lmbd)*((bl_c+eps)/(tauP_l+eps)))
          wr_u = wr_u/(wr_u+wr_c)
          wr_c = wr_c/(wr_u+wr_c)
          wl_u = wl_u/(wl_u+wl_c)
          wl_c = wl_c/(wl_u+wl_c)
          ...
          am = nint(0.5d0*(1.d0-(avel+eps)/abs(avel+eps)))
          fl = u_lcc
          fm = u_mcc
          fc = u_ccc
          fp = u_pcc
          ...
          ! v advection
          !
          avel = 0.25d0*(u_mcc+u_ccc+u_cpc+u_mpc)
          ap = nint(0.5d0*(1.d0+(avel+eps)/abs(avel+eps)))
          am = nint(0.5d0*(1.d0-(avel+eps)/abs(avel+eps)))
          fl = v_lcc
          fm = v_mcc
          ...
          ! w advection
          !
          avel = 0.25d0*(u_mcc+u_ccc+u_ccp+u_mcp)
          ap = nint(0.5d0*(1.d0+(avel+eps)/abs(avel+eps)))
          am = nint(0.5d0*(1.d0-(avel+eps)/abs(avel+eps)))
          fl = w_lcc
          fm = w_mcc
