...
module mod_initflow
  ...
  subroutine initflow(inivel,bcvel,ng,lo,l,dl,zc,zf,dzc,dzf,rho,mu,bforce,is_wallturb,time,u,v,w,p)
    ...
    case('vox') ! 2D serpentine vortex around x
      do k=1,n(3)
        zcc = zc(k)/l(3)
        zff = zf(k)/l(3)
        do j=1,n(2)
          ...
          do i=1,n(1)
            ...
            xf = (i+lo(1)-1-.0)*dl(1)/l(1)
            u(i,j,k) = 0.
            v(i,j,k) =  sin(pi*yf )**2*sin(2*pi*zcc)*cos(pi*time/4.)
            w(i,j,k) = -sin(pi*zff)**2*sin(2*pi*yc )*cos(pi*time/4.)
            p(i,j,k) = 0.!(cos(2.*xc)+cos(2.*yc))*(cos(2.*zcc)+2.)/16.*uref**2
          end do
        end do
      end do
    ...
    if(.not.any(inivel == ['tgv','tgw','rox','roy','roz','vox','voy','voz'])) then
      do k=1,n(3)
        do j=1,n(2)
          do i=1,n(1)
            u(i,j,k) = u1d(k)
    ...
    if(is_mean) then
      if(trim(inivel) /= 'iop') then
        ...
        call set_mean(n,0._rp,dzf/l(3)*(dl(1)/l(1))*(dl(2)/l(2)),u(1:n(1),1:n(2),1:n(3)))
      end if
    end if
    ...
    if(is_pair) then
      ...
      !
      ! alternativel(2), using a Taylor-Green vortex
  ...
  subroutine initscal(inisca,bcsca,ng,lo,l,dl,dzf,zc,s)
    ...
    if(trim(inisca) == 'dhc') then
      do k=1,n(3)
        do j=1,n(2)
          do i=1,n(1)
            ii = i+lo(1)-1
            xx = (ii-0.5)*dl(1)/l(1)
            s(i,j,k) = ((1.-xx)*bcsca(0,1) + xx*bcsca(1,1))
          end do
        end do
      end do
    end if
