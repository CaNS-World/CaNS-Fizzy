...
module mod_rk
  ...
  subroutine rk(rkpar,n,dli,dzci,dzfi,dt,dt_r, &
    ...
    real(rp), intent(in   ), dimension(0:,0:,0:)    :: p,pn,po
    ...
    block
      ...
      do k=1,n(3)
        do j=1,n(2)
          do i=1,n(1)
            rhox_n = rho + drho*0.5*(psio(i,j,k,1)+psio(i+1,j,k,1))
            rhoy_n = rho + drho*0.5*(psio(i,j,k,1)+psio(i,j+1,k,1))
            rhoz_n = rho + drho*0.5*(psio(i,j,k,1)+psio(i,j,k+1,1))
  ...
  subroutine rk_2fl(rkpar,n,dli,dzci,dzfi,dt,gam,seps,u,v,w,normx,normy,normz,phi,psi,rglrx,rglry,rglrz)
    ...
    call clip_field([0._rp,1._rp],psi)
    !
    ! swap d?dtrk <-> d?dtrko
    !
